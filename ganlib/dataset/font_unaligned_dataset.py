...
class FontUnalignedDataset(BaseDataset):
    """
    ...
    def __getitem__(self, index):
        ...
        #assert A_path.split('/')[-1].replace('_fake_B', '') == B_path.split('/')[-1], (A_path, B_path)
        A_img = Image.open(A_path).convert('RGB')
        B_img = Image.open(B_path).convert('RGB')
        w, h = B_img.size
        h2 = int(h/2)
        h -= int(h%2!=0)
        B_img = B_img.crop((0, 0, w, h2))
        ...
        size =[min(A_img.size[0], B_img.size[0]), min(A_img.size[1], B_img.size[1])]
        params = get_params(self.opt, size)
        transform=get_transform(self.opt, params=params, grayscale=(self.input_nc == 1))
        A = transform(A_img)
        transform=get_transform(self.opt, params=params, grayscale=(self.output_nc == 1))
        B = transform(B_img)
