...
class BaseDataset(data.Dataset, ABC):
    ...
    -- <__getitem__>:                   get a dataset point.
    ...
    def __getitem__(self, index):
        ...
        Returns:
            a dictionary of dataset with their names. It ususally contains the dataset itself and its metadata information.
        """
        pass
...
def __scale_width(img, target_width, method=Image.BICUBIC, base=4):
    ...
    h = max(int(round(h / base) * base), base)
    w = max(int(round(w / base) * base), base)
    return img.resize((w, h), method)


def __pad_zero(img, target_width, method=Image.BICUBIC):
    ow, oh = img.size
    if ow > target_width:
        img = img.resize((target_width, oh), method)
    elif ow < target_width:
        # im = Image.new('RGB', (target_width, oh), (0,0,0))
        # im.paste(img)
        pad = Pad(padding=(0, 0, target_width-ow, 0),
                  fill=(0, 0, 0),
...
def __crop(img, pos, size):
    ...
    if (ow-x1 >= tw and oh-y1 >= th):
        #print ('ow >= tw and oh >= th', img.size, (tw,th))
        return img.crop((x1, y1, x1 + tw, y1 + th))
    target = Image.new('RGB', (tw,th), (0,0,0))
